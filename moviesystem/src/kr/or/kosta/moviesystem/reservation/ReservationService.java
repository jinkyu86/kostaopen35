 ...
 */
public class ReservationService implements ModelDriven, ServletContextAware,
	...
	public String viewReservationSeat() throws Exception {
		...
		RESERVATION_LIST = ReservationDAO.selectSeatNumByScrnumAndUserid(
				scrnum, userid);
	...
	public String InsertReservation2() throws Exception {
		...
		System.out.println("mnum : " + scrnum);
		System.out.println("resQty : " + resQty);
		// seatCnt = reservationDAO.selectReservationSeatCount("20","46");
		System.out.println("seatCnt : "
				+ ReservationDAO.selectReservationCount("mmanager"));
	...
	public String SelectSeat() throws Exception {
		...
		System.out.println("seat=" + seat + "count=" + count + "scrnum="
				+ scrnum);
	...
	public String viewReservationByResNumForm() throws Exception {
		...
		long finalSeatNum = RESERVATION.getSeatnum()
				+ (RESERVATION.getResQty() - 1);
	...
	public String viewReservationTimeForm() throws Exception {
		...
		// userid와 mnom으로 group by time 으로 된 정보의 time sum(total_price)
		// count(res_qty)
	...
	public void MovieTimeList() throws Exception {
	...
	private void addReservation(HttpServletRequest request,
		...
		// Reservation에 정보저장
		System.out.println("아이디 " + userid + " 영화번호" + mnum + " 상형관번호" + scrnum
				+ " 수량" + resQty);
		Reservation reservation = new Reservation();
		member = new Member();
		Movie movie = new Movie();
		...
		RequestDispatcher rd = request
				.getRequestDispatcher("/ReservationService?method=viewReservationListById");
		rd.forward(request, response);
	...
	private void test(HttpServletRequest request, HttpServletResponse response)
		...
		RequestDispatcher rd = request
				.getRequestDispatcher("/reservation/test.jsp");
	...
	public String viewReservationListById() throws Exception {
		...
		RESERVATION_LIST = ReservationDAO.selectReservationList(length, page,
				member.getUserid());
		...
		// 페이지에 해당하는 학생수 조회
		int reservationCount = ReservationDAO.selectReservationCount(member
				.getUserid());
		...
		String pageLinkTag = PageUtil.generate(page, reservationCount, length,
